...
- test(account-import-export): 继续扩展 `src/layouts/__tests__/AppHeader.test.ts`，补“导入当前账户时误选整库备份会被拦截且不污染现有本地状态”与“导出当前/全部账户按钮会走对模式并生成对应文件名”两条组件级回归，把账户管理里的文件流高风险接线也纳入真实界面测试
