...
当前结论：
- 主干可正常构建、类型检查与测试通过
- AI 分析相关 UI 与 Markdown/流式逻辑已拆为按需加载，不再进入首屏主包
- 生产构建后主包仍然偏大，主要来自 ECharts / Ant Design Vue / 主业务界面
- 本地存储已抽象为 `StateRepository` 接口，后续可在不大改业务 store 的前提下接入在线存储或登录态
- 开发态 AI 代理已增加目标 URL 白名单约束，避免退化成任意转发代理

建议继续推进：
1. 为图表库和较重页面继续做 manualChunks / 细分拆包
2. 将 AI 对话配置与聊天记录从业务工具中继续分层，准备未来账号隔离
3. 为存储仓储增加第二实现（如 memory / remote stub），验证接口稳定性
4. 补充导入导出与迁移链路测试，降低未来 schema 升级风险
