...
推荐使用一套 Cloudflare 原生无服务器架构：

- **前端与 API**：Cloudflare Workers + Static Assets，同一个 Worker 部署 React SPA 与 `/api/*`。
- **API 框架**：Hono。
...
这套方案没有常驻进程、虚拟机、Docker 服务器或自建 MySQL，初期可以运行在 Cloudflare 免费额度内。
...
| 产品 | 免费额度关键项 | 本项目用途 |
| --- | --- | --- |
| Workers | 100,000 个动态请求/天 | 登录、数据读取与保存 API |
...
| Static Assets | 静态文件随 Worker 单次部署并由 Cloudflare 缓存 | React、CSS、字体等资源 |
...
前端和 API 使用同一域名，可直接使用安全 Cookie，避免跨域、额外网关和两个部署项目。
...
- 引入 Cloudflare Vite 插件、Wrangler、Hono 和 Worker 入口；
- 同一部署提供 SPA 静态资源与 `/api/v1/health`；
- 创建本地与远程 D1 配置；
- 建立 Drizzle migrations；
- CI 执行测试、构建和 migration 检查。

验收：本地 Wrangler 和 Cloudflare 预览环境均可打开现有应用，健康检查正常。
...
建议下一步先开发阶段 A，而不是直接写注册页面。先证明现有 React 应用可以通过一个 Cloudflare Worker 同时提供静态资源、API 和 D1 migration，再接入认证，能最早暴露运行时与依赖兼容问题。

进入阶段 B 前只需要确认一项产品选择：第一期是否接受“邮箱 + 密码 + 邮件验证”为主登录方式。推荐接受；OAuth 和 Passkey 留到后续，不阻塞云端存储主链路。
